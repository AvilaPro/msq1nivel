...
INSERT INTO `alumno_seccion` VALUES (1,1,7,'2019-05-16',99000),(2,2,6,'2019-05-15',85000),(3,3,5,'2019-05-18',101000),(4,4,4,'2019-05-11',90000),(5,5,3,'2019-05-16',95000),(6,6,2,'2019-05-18',101000),(7,5,1,'2019-05-16',80000),(8,6,7,'2019-05-16',75000),(9,7,1,'2019-05-16',101000),(10,8,2,'2019-05-16',110000),(11,9,3,'2019-05-11',99000),(12,10,3,'2019-05-11',95000),(13,4,2,'2019-05-15',85000);
/*!40000 ALTER TABLE `alumno_seccion` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Temporary view structure for view `alumnos_mes_pasado`
--

DROP TABLE IF EXISTS `alumnos_mes_pasado`;
/*!50001 DROP VIEW IF EXISTS `alumnos_mes_pasado`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `alumnos_mes_pasado` AS SELECT 
 1 AS `Seccion`,
 1 AS `Nombre`,
 1 AS `Curso`,
...
--
-- Final view structure for view `alumnos_mes_pasado`
--
...
/*!50001 VIEW `alumnos_mes_pasado` AS select `alumno_seccion`.`seccion_id` AS `Seccion`,`alumno`.`nombre` AS `Nombre`,`curso`.`nombre` AS `Curso`,`alumno_seccion`.`fecha_inscripcion` AS `fecha_inscripcion` from ((`alumno_seccion` join `alumno` on((`alumno_seccion`.`alumno_id` = `alumno`.`id`))) join `curso` on((`alumno_seccion`.`seccion_id` = `curso`.`id`))) where (`alumno_seccion`.`fecha_inscripcion` between '2019-04-01' and '2019-06-01') */;
...
-- Dump completed on 2019-06-22 21:41:12
